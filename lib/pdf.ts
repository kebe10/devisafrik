...
// Fonction de formatage avec devise (compatible avec l'ancienne mise en page)
...
interface PDFQuote {
  ...
  organization: { 
    ...
    email?: string|null; 
    rccm?: string|null; 
    devis_color?: string|null; 
    devis_footer?: string|null;
    currency?: string;   // ← Ajouté
  };
}

export function generateQuotePDF(quote: PDFQuote): void {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const color = quote.organization.devis_color || '#FF6B35';
  const currency = quote.organization.currency || 'XOF';   // ← Récupération de la devise
  const pageW = 210; 
  const pageH = 297; 
  const margin = 14; 
  const cW = 182;
  ...
  doc.setFontSize(14); 
  doc.setFont('helvetica', 'bold');
  doc.text(quote.organization.name.substring(0, 35), margin + 18, 17);
  
  doc.setFontSize(8); 
  doc.setFont('helvetica', 'normal');
  let orgY = 24;
  if (quote.organization.phone) { doc.text(quote.organization.phone, margin + 18, orgY); orgY += 5; }
  if (quote.organization.email) { doc.text(quote.organization.email, margin + 18, orgY); orgY += 5; }
  if (quote.organization.address) doc.text(quote.organization.address.substring(0,50), margin + 18, orgY);
  if (quote.organization.rccm) doc.text('RCCM: ' + quote.organization.rccm, margin + 18, 39);
  ...
  if (quote.title) {
    ...
    doc.text(quote.title.substring(0, 60), margin, y); 
    y += 9;
  }
  ...
  const drawRow = (label: string, value: string, highlight = false) => {
    if (highlight) {
      doc.setFillColor(r, g, b); 
      doc.roundedRect(tX, y, tW, 10, 1, 1, 'F');
      doc.setTextColor(255, 255, 255); 
      doc.setFont('helvetica', 'bold'); 
      doc.setFontSize(10);
    } else {
  ...
  if (quote.payment_terms) {
    ...
    doc.setFont('helvetica', 'normal'); 
    doc.setTextColor(60, 60, 80);
    doc.text(quote.payment_terms, margin, y); 
    y += 8;
  }
