...
export default function ServicesPage() {
  ...
  const loadServices = async (oid: string) => {
    const { data } = await supabase.from('services').select('*').eq('organization_id', oid).order('name')
    setServices(data || [])
  }
  ...
  const saveService = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from('services')
        .update({ ...form, unit_price: Number(form.unit_price) }).eq('id', editing.id)
      if (!error) {
      ...
      const { data, error } = await supabase.from('services')
        ...
        .select().single()
      if (!error && data) { setServices(prev => [data, ...prev]); setAdding(false); resetForm() }
    }
    setSaving(false)
  }
  ...
  const cancelForm = () => { setAdding(false); setEditing(null); resetForm() }
  ...
  return (
    <AppLayout org={org}>
      <div style={{ padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 2 }}>
              {isPremium ? `${services.length} service${services.length > 1 ? 's' : ''} enregistré${services.length > 1 ? 's' : ''}` : 'Fonctionnalité Premium'}
            </p>
          </div>
          {isPremium && (
        ...
        {!isPremium ? (
          <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 16, padding: 48, textAlign: 'center' }}>
            <div style={{ fontSize: 56, marginBottom: 16 }}>🛠️</div>
            <div style={{ fontWeight: 800, fontSize: 18, color: 'var(--blue)', marginBottom: 10 }}>
              Catalogue de services
            </div>
            <div style={{ color: 'var(--text-muted)', fontSize: 14, lineHeight: 1.7, marginBottom: 24, maxWidth: 400, margin: '0 auto 24px' }}>
              Créez votre catalogue de services réutilisables et ajoutez-les en un clic dans vos devis.<br />
              <strong>Disponible avec le plan Premium.</strong>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 300, margin: '0 auto 28px' }}>
              {['Créez vos services une fois', 'Ajoutez-les en 1 clic dans vos devis', 'Organisez par catégorie', 'Prix pré-remplis automatiquement'].map(f => (
            ...
            <button onClick={() => router.push('/subscription')}
              style={{ padding: '12px 28px', background: 'var(--orange)', color: '#fff', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer', border: 'none' }}>
              ⭐ Passer Premium →
            </button>
          </div>
        ) : (
                ...
                {filtered.map(s => (
                  <div key={s.id} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 14, padding: 16, boxShadow: 'var(--shadow-sm)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => startEdit(s)}
                          style={{ background: '#F0F4FF', border: 'none', color: 'var(--blue)', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 12 }}>✏️</button>
                        <button onClick={() => deleteService(s.id)}
                          style={{ background: '#FEF2F2', border: 'none', color: '#DC2626', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 12 }}>🗑️</button>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontWeight: 800, fontSize: 16, color: 'var(--orange)' }}>{formatAmount(s.unit_price, currency)}</span>
                      <span style={{ background: '#F8F9FA', color: 'var(--text-muted)', fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>par {s.unit}</span>
                    </div>
                  </div>
                ))}
              </div>
